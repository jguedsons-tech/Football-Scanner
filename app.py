...
def main():
    ...
    with st.sidebar:
        ...
        days = st.slider("Dias para pesquisar", 1, 7, 2)
    ...
    with auto:
        st.write(f"Procura jogos de hoje até **{days} dia(s)** à frente.")
    ...
    with manual:
        ...
        if st.button("🔍 Procurar jogo", type="primary", use_container_width=True):
            if not q.strip():
                ...
                try:
                    ms = search_games(key, q, date.today(), date.today() + timedelta(days=7))
                    if not ms:
                        st.warning("Jogo não encontrado nos próximos 7 dias.")
                    else:
