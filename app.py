...
@st.cache_data(ttl=1800)
def stats(team,league,season=SEASON):
...
def avg(s,kind,side):
    ...
    return f(v,0)
def model(g):
    t=g['teams']; l=g['league']; pr=prediction(g['fixture']['id']); hs=stats(t['home']['id'],l['id']) if l.get('id') else {}; a_s=stats(t['away']['id'],l['id']) if l.get('id') else {}
